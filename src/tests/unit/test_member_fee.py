...
def member_membership(new_member):
    ...
    yield ms
    Transaction.objects.filter(bookings__member=ms.member).delete()
    ms.delete()
...
def test_liabilities_easy(member_membership):
    ...
    credits = member_membership.member.bookings.filter(booking_type=BookingType.CREDIT).all()
...
def test_liabilities_future_transactions(member_membership):
    ...
    member_membership.member.update_liabilites()
    bookings = member_membership.member.bookings.filter(booking_type=BookingType.CREDIT).all()
...
def test_liabilities_complicated_example(member_membership, member_membership_second):
    member_membership.member.update_liabilites()
    virtual_transactions = member_membership.member.bookings.filter(booking_type=BookingType.CREDIT).all()
